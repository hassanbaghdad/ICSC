<template>
    <div>
        <v-dialog v-model="$store.state.dash.labs.forms.add_lab" max-width="900">
            <v-card>
                <v-card-title class="success rtl" style="width: 100%">
                    <v-icon dark>mdi-plus</v-icon>
                    <span class="mr-2 cf white--text">اضافة معمل</span>
                </v-card-title>
                <v-card-text>
                       ...
                       <div class="row">
                           <div class="col col-12 col-lg-8">
                               <v-card min-height="200" class="text-center">
                                   ...
                                   <v-img v-if="src_cover!=null&& src_cover!=''" :src="src_cover" contain max-height="300"></v-img>
                               </v-card>
                               <v-file-input prepend-icon="mdi-image-plus" label="اختيار صورة غلاف" ref="photo" :rules="req" @change="image_change_cover" v-model="lab.lab_image_cover" />
                           ...
                           <div class="col col-12 col-lg-4">
                               <v-card min-height="200" class="text-center">
                                   ...
                                   <v-img v-if="src!=null&& src!=''" :src="src" contain max-height="300"></v-img>
                               </v-card>
                               <v-file-input prepend-icon="mdi-image-plus" label="اختيار صورة منتج" ref="photo" :rules="req" @change="image_change" v-model="lab.lab_image" />
                ...
                <v-card-actions class="rtl">
                    ...
                    <v-btn outlined class="mr-4"  @click="$store.state.dash.labs.forms.add_lab=false;$refs.form.reset();src=''">
    ...
    export default {
        ...
        data(){
            return{
                ...
                req_no:[v=>!!v&&Number(v) || 'يرجى ادخال رقم'],
                lab:{
                    lab_name:'',
                    lab_name_en:'',
                    lab_location:'',
                    ...
                    lab_prod_type:'',
                    lab_prod_type_en:'',
                    lab_count_prod_lines:'',
                    lab_design_energy:'',
                    lab_establishment_date:'',
                    lab_start_date:'',
                    lab_company:'',
                    lab_company_en:'',
                    lab_manufacturing_method:'',
                    lab_manufacturing_method_en:'',
                    lab_cement_type:'',
                    lab_image:null,
                    lab_image_cover:null,
        ...
        methods:{
            ...
            async upload(){
                if(this.$refs.form.validate())
                {
                    ...
                    this.$axios.post('api-dash/add-lab',fm).then(res=>{
                        this.$fire({
                            title: "نجح",
                            text: res.data.msg,
                            type: "success",
                        ...
                        this.$store.state.dash.labs.forms.add_lab = false;
